...
def dynamicMethods(nodeAttribute,nodeNeighbors,initsize,evolutionmodel,methods,r,coverS,edges=None,avedeg=None,opinion=None,static=False,duration=3):
    count=0
    Cost=[0,0]   #jump, walk
    a = 0
    interval = 1
...
def Random2(evolutionmodel,startsize):
    ...
    for r in range(3,2,-1):
        ...
        for i in range(1):
            # poolist1.append(p1.apply_async(cm.Random, args=(copy.deepcopy(nodeAttribute),copy.deepcopy(nodeNeighbors),coverS,evolutionmodel,startsize,)))
            poolist1.append(cm.Random(copy.deepcopy(nodeAttribute),copy.deepcopy(nodeNeighbors),coverS,evolutionmodel,startsize,None,6,Opinion,False))
        p1.close()
        p1.join()
        sumlist=[]
        opinionlist=[]
