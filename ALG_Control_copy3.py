...
def test2(evolutionmodel, initsize, methods, duration=3):
    """
    ...
    for r in range(2, 1, -1):
        ...
        for i in range(2):
            poolist1.append(p1.apply_async(dynamicMethods, args=(
                copy.deepcopy(nodeAttribute), copy.deepcopy(nodeNeighbors), initsize, evolutionmodel, methods, r,
                coverS,
...
def dynamicMethods(nodeAttribute, nodeNeighbors, initsize, evolutionmodel, methods, r, coverS, edges=None, avedeg=None,
                   opinion=None, static=False, duration=3):
    count = 0
    Cost = [0, 0]  # jump, walk
    a = 0
    interval = 1
    while len(coverS.CS) < len(nodeAttribute.keys()):
        ...
        # if coverS.opinion.ComputeOpinion(nodeAttribute) > 0.8:
        #     break
        # if coverS.opinion.rounds and len(coverS.bs) <10:
        # if coverS.opinion.rounds and len(coverS.bs) >= 20:
        #     if len(coverS.bs) in range(20,210,10):
        #         tt = opinion.ComputeRounds(nodeNeighbors, coverS)
        #         print(tt)
        #         coverS.RoundsList.append(tt)
            # if tt:
            #     coverS.RoundsList.append(tt)
            # if len(coverS.bs) > 52: break
            # if len(coverS.bs)>202: break
            # else: break
    print("count:%d" % count)
...
def statiCases(evolutionmodel, endsize, r=100):
    ...
    for i in range(1):
        ...
        coverS = cm.coverSet(r)
        coverS.opinion = op
        poolist1.append(p1.apply_async(cm.AlternateRandom2, args=(
        copy.deepcopy(nodeAttribute), copy.deepcopy(nodeNeighbors), copy.deepcopy(coverS), None, op, None, None)))
    p1.close()
    p1.join()
    sizelist = []
    opinionList = []
...
def Random(evolutionmodel, startsize):
    ...
    for r in range(2, 1, -1):
        ...
        for i in range(1):
            ...
            poolist1.append(
                cm.Random(copy.deepcopy(nodeAttribute), copy.deepcopy(nodeNeighbors), coverS, evolutionmodel, startsize,
                          op, None, 6, True))
        p1.close()
        p1.join()
